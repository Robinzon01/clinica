...
@Component
public class PageInitPagDetailConsul {

	@Autowired
	private IDetailConsultationService detailConsulService;
	...
	public  ModelAndView initPagination(Optional<Integer> pageSize, Optional<Integer> page, String url) {
		...
		Page<DetailConsultation> detailConsulList = detailConsulService.findAll(PageRequest.of(evalPage, evalPageSize));
		PagerModel pager = new PagerModel(detailConsulList.getTotalPages(), detailConsulList.getNumber(), BUTTONS_TO_SHOW);

		initModelView.addObject("detailConsulList", detailConsulList);
