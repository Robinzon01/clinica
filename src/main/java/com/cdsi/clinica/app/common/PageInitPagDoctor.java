...
@Component
public class PageInitPagDoctor {
	...
	public  ModelAndView initPagination(Optional<Integer> pageSize, Optional<Integer> page, String url) {
		...
		Page<Doctor> doctorList = doctorService.findAll(PageRequest.of(evalPage, evalPageSize));
		PagerModel pager = new PagerModel(doctorList.getTotalPages(), doctorList.getNumber(), BUTTONS_TO_SHOW);

		initModelView.addObject("doctorList", doctorList);
